#!/usr/bin/env bash
# Setup virtual environments for bootstrap and launcher.
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --bootstrap-only) SETUP_LAUNCHER=false; shift ;;
    --launcher-only)  SETUP_BOOTSTRAP=false; shift ;;
    --all)            SETUP_BOOTSTRAP=true; SETUP_LAUNCHER=true; shift ;;
    -h|--help)
      echo "Usage: bash bootstrap/setup-venvs.sh [--python <exe>] [--bootstrap-only|--launcher-only|--all]"
      exit 0
      ;;
