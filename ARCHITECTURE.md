...
Deploy merges `VARS` + `SECRETS` into Lambda/ECS runtime except `RUNTIME_ENV_EXCLUDE` (e.g. `AWS_GITHUB_OIDC_ROLE_ARN` for CI only). Lambda create/update metadata is fixed in `extensions-service/deploy_input.py`; `FunctionName` comes from `VARS.LAMBDA_HANDLERS_FUNCTION_NAME`.
